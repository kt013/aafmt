...
\subsection*{Do the subsets of a set form a group?} 
Some of the properties in Proposition~\ref{proposition:sets:sets_theorem_set_ops} may ring a bell. Recall that in the Integers Mod $n$ section of the Modular Arithmetic chapter we defined a \bfii{ group} to be a set combined with an operation that has the following properties:
\begin{enumerate}
\item
...
\item
**If you listed the subsets of $\{a,b,c,\ldots,x,y,z\}$, how many subsets would be in your list?  \emph{(Hint: guess the pattern from the previous parts of this exercise.)}
\end{enumerate}
\end{exercise}
